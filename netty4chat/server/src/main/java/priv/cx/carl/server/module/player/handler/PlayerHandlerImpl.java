...
import priv.cx.carl.server.module.player.service.PlayerService;
...
public class PlayerHandlerImpl implements PlayerHandler {
    ...
    public Result<PlayerResponse> registerAndLogin(Session session, byte[] data) {
        ...
        try {
            ...
            //参数判空
            if (StringUtils.isEmpty(registerRequest.getPlayerName()) || StringUtils.isEmpty(registerRequest
					.getPassward())) {
                return Result.ERROR(ResultCode.PLAYERNAME_NULL);
            }

            //执行业务
            result = playerService.registerAndLogin(session, registerRequest.getPlayerName(), registerRequest
					.getPassward());
